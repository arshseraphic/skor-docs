...
When you create a webhook, you can provide an optional shared `secret` for that URL. If a secret is configured, SKOR includes it in the `x-skor-signature` header on every webhook POST request. Compare this header with the stored secret before accepting the event.
