...
Generate a dedicated property management token with the landlord's email and password. Send the email as the HTTP Basic Auth username and the password as the HTTP Basic Auth password.

```bash
curl --request POST \
  --user 'landlord@example.com:secret' \
  'https://api.skortorent.com/api/v1/property-management/auth/token'
```

Do not include the landlord credentials in a JSON body. The response includes the token type, expiration time, and a bearer token that remains valid for 24 hours.
...
<Steps>
  <Step title="Generate a property management token">
    Authenticate with the landlord's email and password through HTTP Basic Auth. Store the returned bearer token securely.
  </Step>
