...
<Note>
  The Modular API and Property Management API use the same token endpoint.
</Note>
...
<Steps>
  <Step title="Generate a shared access token">
    Authenticate at `/authenticate/token` with your `client_id` and `client_secret`. Store the returned one-hour bearer token securely.
  </Step>
