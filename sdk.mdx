...
<Note>
  ...
  only for server-side calls and must never be exposed in browser code. See
...
Using your Bearer access token (see [SDK authentication](/sdk-auth)):

1. Create the tenant with **Create Tenant** and keep the returned `id` (the `tenantId`).
2. Generate an onboarding token for that tenant:
...
The SDK automatically tracks the session's progress (via `/tenants/account-status`) and drives the callbacks below. You do not need to call the status endpoint yourself.
